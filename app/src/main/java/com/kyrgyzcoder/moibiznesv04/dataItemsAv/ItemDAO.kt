...
interface ItemDAO {
    ...
    @Query("SELECT * FROM items_available_table WHERE itemCategory==1111 ORDER BY id ASC")
    fun getVesnaItems(): LiveData<List<Item>>

    @Query("SELECT * FROM items_available_table WHERE itemCategory==2222 ORDER BY id ASC")
    fun getLetoItems(): LiveData<List<Item>>

    @Query("SELECT * FROM items_available_table WHERE itemCategory==3333 ORDER BY id ASC")
    fun getOsenItems(): LiveData<List<Item>>

    @Query("SELECT * FROM items_available_table WHERE itemCategory==4444 ORDER BY id ASC")
    fun getShkolnyiItems(): LiveData<List<Item>>

    @Query("SELECT * FROM items_available_table WHERE itemCategory==5555 ORDER BY id ASC")
    fun getZimaItems(): LiveData<List<Item>>

    @Query("SELECT * FROM items_available_table WHERE itemCategory==6666 ORDER BY id ASC")

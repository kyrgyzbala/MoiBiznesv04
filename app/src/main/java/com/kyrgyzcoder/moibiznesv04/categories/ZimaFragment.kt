...
import kotlinx.android.synthetic.main.fragment_zima.*

/**
 * A simple [Fragment] subclass.
 */
class ZimaFragment : Fragment(), ItemsRecyclerViewAdapter.OnItemClickListenerN {

    private lateinit var viewModel: AllItemsViewModel
    private lateinit var adapterZ: ItemsRecyclerViewAdapter
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_zima, container, false)
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        viewModel = ViewModelProviders.of(this).get(AllItemsViewModel::class.java)
        recyclerViewZima.layoutManager = LinearLayoutManager(this.context)
        recyclerViewZima.setHasFixedSize(true)

        adapterZ = ItemsRecyclerViewAdapter(this)
        recyclerViewZima.adapter = adapterZ

        viewModel.getZimaItems().observe(viewLifecycleOwner, Observer {
            adapterZ.submitList(it)
        })

        fabZima.setOnClickListener {
    ...
    private fun onSwipeDelete() {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                dialogBuilder.setMessage("Вы уверены что хотите удалить этот товар? Вы не сможете вернуть обратно")
                    ...
                    .setPositiveButton("Удалить") { _, _ ->
                        viewModel.deleteItem(adapterZ.getItemAt(viewHolder.adapterPosition))
        ...
        }).attachToRecyclerView(recyclerViewZima)
    }
}

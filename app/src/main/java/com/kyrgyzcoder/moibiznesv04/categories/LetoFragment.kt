...
import kotlinx.android.synthetic.main.fragment_leto.*

/**
 * A simple [Fragment] subclass.
 */
class LetoFragment : Fragment(), ItemsRecyclerViewAdapter.OnItemClickListenerN {

    private lateinit var viewModel: AllItemsViewModel
    private lateinit var adapterL: ItemsRecyclerViewAdapter
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_leto, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        viewModel = ViewModelProviders.of(this).get(AllItemsViewModel::class.java)
        recyclerViewLeto.layoutManager = LinearLayoutManager(this.context)
        recyclerViewLeto.setHasFixedSize(true)

        adapterL = ItemsRecyclerViewAdapter(this)
        recyclerViewLeto.adapter = adapterL

        viewModel.getLetoItems().observe(viewLifecycleOwner, Observer {
            adapterL.submitList(it)
        })

        fabLeto.setOnClickListener {
    ...
    private fun onSwipeDelete() {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                dialogBuilder.setMessage("Вы уверены что хотите удалить этот товар? Вы не сможете вернуть обратно")
                    ...
                    .setPositiveButton("Удалить") { _, _ ->
                        viewModel.deleteItem(adapterL.getItemAt(viewHolder.adapterPosition))
        ...
        }).attachToRecyclerView(recyclerViewLeto)
    }
}

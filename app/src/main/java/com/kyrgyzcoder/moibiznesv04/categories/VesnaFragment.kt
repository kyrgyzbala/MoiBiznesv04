...
import kotlinx.android.synthetic.main.vesna_fragment.*

class VesnaFragment : Fragment(), ItemsRecyclerViewAdapter.OnItemClickListenerN {

    private lateinit var viewModel: AllItemsViewModel
    private lateinit var adapterV: ItemsRecyclerViewAdapter
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.vesna_fragment, container, false)
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        viewModel = ViewModelProviders.of(this).get(AllItemsViewModel::class.java)
        recyclerViewVesna.layoutManager = LinearLayoutManager(this.context)
        recyclerViewVesna.setHasFixedSize(true)

        adapterV = ItemsRecyclerViewAdapter(this)
        recyclerViewVesna.adapter = adapterV

        viewModel.getVesnaItems().observe(viewLifecycleOwner, Observer {
            adapterV.submitList(it)
        })

        fabVesna.setOnClickListener {
    ...
    private fun onSwipeDelete() {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                dialogBuilder.setMessage("Вы уверены что хотите удалить этот товар? Вы не сможете вернуть обратно")
                    ...
                    .setPositiveButton("Удалить") { _, _ ->
                        viewModel.deleteItem(adapterV.getItemAt(viewHolder.adapterPosition))
        ...
        }).attachToRecyclerView(recyclerViewVesna)
    }
}

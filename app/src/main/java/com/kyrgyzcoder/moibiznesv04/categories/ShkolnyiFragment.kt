...
import kotlinx.android.synthetic.main.fragment_shkolnyi.*

/**
 * A simple [Fragment] subclass.
 */
class ShkolnyiFragment : Fragment(), ItemsRecyclerViewAdapter.OnItemClickListenerN {
    private lateinit var viewModel: AllItemsViewModel
    private lateinit var adapterS: ItemsRecyclerViewAdapter
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_shkolnyi, container, false)
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        viewModel = ViewModelProviders.of(this).get(AllItemsViewModel::class.java)
        recyclerViewShkolnyi.layoutManager = LinearLayoutManager(this.context)
        recyclerViewShkolnyi.setHasFixedSize(true)

        adapterS = ItemsRecyclerViewAdapter(this)
        recyclerViewShkolnyi.adapter = adapterS

        viewModel.getShkolnyiItems().observe(viewLifecycleOwner, Observer {
            adapterS.submitList(it)
        })

        fabShkolnyi.setOnClickListener {
    ...
    private fun onSwipeDelete() {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                dialogBuilder.setMessage("Вы уверены что хотите удалить этот товар? Вы не сможете вернуть обратно")
                    ...
                    .setPositiveButton("Удалить") { _, _ ->
                        viewModel.deleteItem(adapterS.getItemAt(viewHolder.adapterPosition))
        ...
        }).attachToRecyclerView(recyclerViewShkolnyi)
    }
}
